...
class Payment extends CI_Controller {
	...
	public function do_search_receipt()
	{
		...
		} //else
		{
			...
			if($outgoing == TRUE)
			{
				...
				redirect('cashier/payment/payment_receipt_outgoing/' . $search);
			}
			else
			{
				# redirect to outgoing
				redirect('cashier/payment/payment_receipt_incoming/' . $search);
			}
		}
	}
	...
	{		
		...
		if($search == 0){redirect('cashier/payment');}
	...
	public function payment_receipt_incoming($search)
	{
		...
		$paid = $this->cashier_model->get_payment_status_incoming($search);
			...
			else
			{
				...
				# get outgoing dtbarang
				$data['incoming'] = $this->cashier_model->get_dtbarang_incoming($search);
				
				foreach ($data['incoming'] as $item) :
					$no_btb = $item->in_btb;
					$agent = $item->in_agent;
					$no_smu = $item->in_smu ;
